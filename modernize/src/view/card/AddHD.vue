...
import type { CardTransaction, StatsItem } from '@/types/custom/DataTableTypes';
...
const setUsers = (userList: CardTransaction[]) => {
...
// mockApi 로 데이터 불러오기.
const users = ref<CardTransaction[]>([]); // 사용자 데이터를 저장할 변수
const stats = ref<StatsItem[]>([]); // 집계 데이터를 저장할 변수
onMounted(async () => {
    // 초기화 또는 초기 작업 수행
    try {
        const response = await apiClient.get('/card/list/hyundai');
        setUsers(response.list);
        if (response.list) {
            const response = await apiClient.post('/card/usageTypeStats/hyundai', { payDate });
...
const onSearchStats = async (validateForm: any) => {
    ...
    const response = await apiClient.post('/card/usageTypeStats/hyundai', formData);
    setStats(response.list);
};
...
const { onSearch, resetSearch, filteredList, selectedEmpId, onSelectionChange, edit, handleEdit, onNew, onSave, onDelete, onExcelSave } =
    useTableManager<CardTransaction>(users, formFields, userFields, identifierField);
</script>
<!-- 행이 아닌 체크박스만 동작함 -->
<template>
    <v-row>
        <v-col cols="12" md="12">
            <UiParentCard title="현대카드">
                <v-row>
                ...
                <v-row>
                    <v-col cols="12" sm="6">
                        <div class="d-flex gap-3 flex-column flex-wrap flex-xl-nowrap flex-sm-row fill-height">
                            ...
                            <v-btn flat color="error" variant="outlined" @click="onDelete(selectedEmpId[0], '/card/delete/hyundai')"
                    ...
                    <v-col cols="12" sm="6">
                        <div class="d-flex gap-3 justify-end flex-column flex-wrap flex-xl-nowrap flex-sm-row fill-height">
                            ...
                            <ExcelUploadDialogBtn :save="onExcelSave" :url="'/card/saveList/hyundai'" title="엑셀 업로드" />
                        </div>
                    </v-col>
                </v-row>
                <v-row>
                    ...
                    <CustomSearchChecksForm :formFields="userFields" :colsPerRow="4" :edit="edit">
                        <template v-slot:topBtn="{ validateForm }">
                            <v-row>
                                ...
                                <v-col class="v-col-12 v-col-sm-6">
                                    <div class="d-flex gap-3 justify-end flex-column flex-wrap flex-xl-nowrap flex-sm-row fill-height">
                                        <v-btn flat color="primary" variant="outlined" @click="onSave(() => saveToServer(validateForm, '/card/save/hyundai'))" >
                                            저장
                                        </v-btn>
                                    </div>
                                </v-col>
