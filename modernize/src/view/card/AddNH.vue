...
onMounted(async () => {
    // 초기화 또는 초기 작업 수행
    try {
        const response = await apiClient.get('/card/list/nh');
        setUsers(response.list);
        if (response.list) {
            const response = await apiClient.post('/card/usageTypeStats/nh', { payDate });
...
const onSearchStats = async (validateForm: any) => {
    ...
    const response = await apiClient.post('/card/usageTypeStats/nh', formData);
    setStats(response.list);
};
                ...
                <v-row>
                    <v-col>
                        <div class="d-flex gap-3 flex-column flex-wrap flex-xl-nowrap flex-sm-row fill-height">
                            ...
                            <v-btn flat color="error" variant="outlined" @click="onDelete(selectedEmpId[0], '/card/delete/nh/'+selectedEmpId[0])"
                    ...
                    <v-col>
                        <div class="d-flex gap-3 justify-end flex-column flex-wrap flex-xl-nowrap flex-sm-row fill-height">
                            ...
                            <ExcelUploadDialogBtn :save="onExcelSave" :url="'/card/saveList/nh'" title="엑셀 업로드" />
                        </div>
                    </v-col>
                </v-row>
                <v-row>
                                ...
                                <v-col cols="7">
                                    <div class="d-flex gap-3 justify-end flex-column flex-wrap flex-xl-nowrap flex-sm-row fill-height">
                                        <v-btn
                                            ...
                                            color="primary"
                                            variant="outlined"
                                            @click="onSave(() => saveToServer(validateForm, '/card/save/nh'))"
                                        >
                                            저장
                                        </v-btn>
                                    </div>
