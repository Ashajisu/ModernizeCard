...
//집계 검색
const payDate = format(new Date().setDate(14), 'yyyy-MM-dd'); // 매월 결제일 14일 가정.
...
onMounted(async () => {
    // 초기화 또는 초기 작업 수행
    try {
        const response = await apiClient.get('/card/list/kookmin');
        setUsers(response.list);
        if (response.list) {
            const response = await apiClient.post('/card/usageTypeStats/kookmin', { payDate });
...
const onSearchStats = async (validateForm: any) => {
    ...
    const response = await apiClient.post('/card/usageTypeStats/kookmin', formData);
    setStats(response.list);
};
                ...
                <v-row>
                    ...
                    <v-col>
                        <div class="d-flex gap-3 justify-end flex-column flex-wrap flex-xl-nowrap flex-sm-row fill-height">
                            ...
                            <ExcelUploadDialogBtn :save="onExcelSave" :url="'/card/saveList/kookmin'" title="엑셀 업로드" />
                        </div>
                    </v-col>
                </v-row>
                <v-row>
                    ...
                    <CustomSearchChecksForm :formFields="userFields" :colsPerRow="4" :edit="edit">
                        <template v-slot:topBtn="{ validateForm }">
                            <v-row>
                                ...
                                <v-col cols="7">
                                    <div class="d-flex gap-3 justify-end flex-column flex-wrap flex-xl-nowrap flex-sm-row fill-height">
                                        <v-btn flat color="primary" variant="outlined" @click="onSave(() => saveToServer(validateForm, '/card/save/kookmin'))" >
                                            저장
                                        </v-btn>
                                    </div>
                                </v-col>
